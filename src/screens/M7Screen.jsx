...
export default function M7Screen({ progress, onComplete, onBack, lang = 'pt', t = k => k }) {
  const { M7_LESSONS, M7_FINAL_CHALLENGE } = useContent(lang);
  const done = progress?.m7 || [];
  const [lessonIdx, setLessonIdx] = useState(null);
  const [showFinal, setShowFinal] = useState(false);

  if (lessonIdx === null && !showFinal) {
    const allDone = done.length >= M7_LESSONS.length;
    return (
      <div className='sq-module-root' style={{ minHeight:'100dvh', background:'#131f24', display:'flex', flexDirection:'column' }}>
        <div className="sq-mobile-only" style={{ background:'#1c2b32', borderBottom:"1px solid rgba(255,255,255,0.06)", padding:'14px 20px', display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <div style={{ fontFamily:F.mono, color:C.textDim, fontSize:13, letterSpacing:2 }}>MÓDULO 7</div>
            <div style={{ fontFamily:F.display, color:C.text, fontSize:16, fontWeight:900, display:'flex', alignItems:'center', gap:8 }}>Google Threat Intelligence</div>
          </div>
        </div>
        <div style={{ flex:1, overflowY:'auto', padding:'16px 16px 80px', maxWidth:600, width:'100%', margin:'0 auto' }}>
          ...
          <div style={{ padding:'16px 0 20px', borderBottom:`1px solid ${C.border}`, marginBottom:16 }}>
            <div style={{ fontFamily:F.mono, color:C.textDim, fontSize:13, letterSpacing:3, marginBottom:4 }}>MÓDULO 7</div>
            <div style={{ fontFamily:F.display, color:C.text, fontSize:22, fontWeight:900, letterSpacing:-0.3 }}>Google Threat Intelligence</div>
            <div style={{ fontFamily:F.mono, color:C.textDim, fontSize:13, marginTop:6 }}>
              GTI · VirusTotal · Mandiant · IOCs · Threat Hunting
            </div>
          </div>

          {M7_LESSONS.map((lesson, idx) => {
            ...
            return (
              <div key={idx} onClick={() => setLessonIdx(idx)}
                style={{ background:'#1c2b32', border:`2px solid ${isDone ? '#00c4cc55' : C.border}`,
                  borderBottom:`4px solid ${isDone ? '#00c4cc' : C.cardDepth}`,
          ...
          {allDone && (
            <div onClick={() => setShowFinal(true)}
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:F.display, color:'#00c4cc', fontSize:13, fontWeight:800 }}>Desafio Final</div>
  ...
  const handleLessonComplete = (xpEarned) => {
    ...
    onComplete({ m7: newDone }, xpEarned, newDone.length >= M7_LESSONS.length);
    setLessonIdx(null);
  };

  const handleFinalComplete = (xpEarned) => {
    onComplete({ m7: done }, xpEarned, true);
    setShowFinal(false);
  };

  return (
    <ModuleScreen
      lesson={showFinal ? M7_FINAL_CHALLENGE : M7_LESSONS[lessonIdx]}
      isFinal={showFinal}

...
export const M7_LESSONS = [
  { id:1, icon:"🔍", title:"O que é o Google Threat Intelligence",
    cards:[
      { q:"O que é o Google Threat Intelligence (GTI)?", a:"Plataforma unificada de Threat Intelligence do Google que combina: Mandiant Intelligence (pesquisa de ameaças do mundo real), VirusTotal (análise de arquivos/URLs/IPs/domínios) e Google Safe Browsing. Integrada nativamente ao Google SecOps para enriquecimento automático de alertas e IOCs." },
  ...
  { id:3, icon:"🔗", title:"GTI no Google SecOps — Integração",
    cards:[
      { q:"Como o GTI enriquece eventos no Google SecOps?", a:"Via GTI Feed integrado: eventos UDM com IPs, domínios ou hashes são comparados automaticamente com a base GTI. Campos adicionados: $e.metadata.threat.threat_feed_name, $e.metadata.threat.confidence_score, $e.metadata.threat.severity_string, $e.network.ip_location.threat_feeds[]. Sem necessidade de lookup manual." },
      { q:"Como usar IOCs do GTI em regras YARA-L?", a:"Acesse via campos de threat enrichment no UDM:\n\nrule gti_malicious_ip {\n  meta: author = \"SecOps Quest\"\n  events:\n    $e.metadata.event_type = \"NETWORK_CONNECTION\"\n    $e.metadata.threat.confidence_score > 80\n    $e.metadata.threat.severity_string = \"HIGH\"\n  condition:\n    $e\n}" },
    ...
    challenges:[
      { type:"complete", sentence:"O campo UDM que armazena o score de confiança do GTI é $e.metadata.threat.____.", blank:"confidence_score", hint:"confidence_score indica o grau de certeza que aquele IOC é malicioso (0-100). Outros campos: severity_string (LOW/MEDIUM/HIGH/CRITICAL), threat_feed_name (nome do feed), category (C2, MALWARE, PHISHING). Ref: cloud.google.com/chronicle/docs/reference/udm-field-list", options:["confidence_score","trust_level","ioc_score","threat_level"] },

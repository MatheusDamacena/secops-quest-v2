...
const M1_FINAL_CHALLENGE = [
  ...
  { type:"complete", sentence:"Para instalar parsers, playbooks e integrações prontas no SecOps, usa-se o ____.", blank:"Content Hub", hint:"Content Hub: Integrations, Playbooks prontos, Parsers, Dashboards, Use Cases. Instalável em 1 clique, atualizado automaticamente. Ref: community Step 2.2 Content Hub", options:["Content Hub","App Store","API Hub","Google Play"] },
...
// ─── MÓDULO 7 — GOOGLE THREAT INTELLIGENCE ────────────────────────────────────
export const M7_LESSONS = [
  { id:1, icon:"🔍", title:"O que é o Google Threat Intelligence",
    cards:[
      { q:"O que é o Google Threat Intelligence (GTI)?", a:"Plataforma unificada de Threat Intelligence do Google que combina: Mandiant Intelligence (pesquisa de ameaças do mundo real com base em resposta a incidentes e análise de malware) e VirusTotal (análise de arquivos/URLs/IPs/domínios via múltiplos antivírus). Integrada nativamente ao Google SecOps para enriquecimento automático de alertas e IOCs." },
      { q:"Qual a diferença entre GTI e VirusTotal?", a:"VirusTotal é um componente do GTI focado em análise de arquivos, URLs, IPs e domínios via múltiplos antivírus. GTI é mais amplo: inclui relatórios de ameaças, perfis de atores, campanhas, TTPs do MITRE e inteligência do Mandiant. VirusTotal = análise técnica de artefatos. GTI = inteligência estratégica e operacional completa." },
      { q:"O que são IOCs e por que são centrais no GTI?", a:"IOC = Indicator of Compromise. Artefatos observáveis que indicam comprometimento: endereços IP, domínios, hashes de arquivo, URLs, endereços de e-mail. O GTI mantém um banco global de IOCs com contexto de malware, atores e campanhas associadas. No SecOps, IOCs do GTI enriquecem automaticamente eventos UDM." },
      { q:"O que é o GTI Feed e como ele funciona no SecOps?", a:"Feed automático de IOCs do GTI injetado no Google SecOps. Quando um evento UDM contém um IP, domínio ou hash correspondente ao GTI Feed, o SecOps adiciona metadados de threat intelligence ao evento — malware name, confidence score, severity, actor. Permite regras YARA-L que usam esses campos sem lookup manual." },
    ],
    challenges:[
      { type:"complete", sentence:"O componente do GTI responsável por análise de arquivos e URLs via múltiplos antivírus é o ____.", blank:"VirusTotal", hint:"VirusTotal analisa artefatos técnicos (arquivos, URLs, IPs, hashes) usando dezenas de engines antivírus. É o componente técnico do GTI. Mandiant traz inteligência estratégica de atores e campanhas. Ref: cloud.google.com/threat-intelligence", options:["VirusTotal","Mandiant","Safe Browsing","GTI Feed"] },
      { type:"truefalse", statement:"O GTI Feed injeta automaticamente contexto de threat intelligence nos eventos UDM do Google SecOps.", answer:true, hint:"Correto. O GTI Feed enriquece eventos em tempo real — quando um IP ou hash é identificado como IOC, o SecOps adiciona metadados como malware_name, confidence_score e associated_actor diretamente no evento." },
      { type:"complete", sentence:"IOC significa Indicator of ____, artefatos que indicam comprometimento de um sistema.", blank:"Compromise", hint:"IOC = Indicator of Compromise. Exemplos: IPs maliciosos, domínios C2, hashes de malware, URLs de phishing. O GTI mantém milhões de IOCs com contexto de atores e campanhas associadas.", options:["Compromise","Control","Command","Configuration"] },
    ]
  },
  { id:2, icon:"🦠", title:"Malware, Atores e Campanhas",
    cards:[
      { q:"O que é um Threat Actor no contexto do GTI?", a:"Indivíduo ou grupo responsável por atividades maliciosas. O GTI categoriza atores em: Nation-state (ex: APT41 — China), Financially motivated (ex: FIN7 — fraude financeira), Hacktivist, Cybercriminal. Cada perfil inclui TTPs, alvos preferenciais, malware usado e histórico de campanhas." },
      { q:"Diferença entre APT, FIN e UNC no Mandiant:", a:"APT (Advanced Persistent Threat): ator patrocinado por estado, motivação política/espionagem. Ex: APT29 (Cozy Bear — Rússia). FIN: ator financeiramente motivado. Ex: FIN8 (varejo/hospitality). UNC (Uncategorized): ator com atividade observada mas origem ainda não confirmada. Quando confirmado, recebe designação APT ou FIN." },
      { q:"O que é uma campanha no GTI?", a:"Série de ataques relacionados usando TTPs, infraestrutura e objetivos similares, atribuídos a um mesmo ator ou cluster de atividade. O GTI documenta campanhas com IOCs específicos, linha do tempo, alvos e malware usado. No SecOps, correlacionar eventos com campanhas ativas ajuda a priorizar resposta." },
      { q:"O que é malware family e como o GTI categoriza?", a:"Família de malware = conjunto de variantes com código-base comum. GTI categoriza por: tipo (ransomware, backdoor, stealer, loader, RAT, wiper), ator associado, funcionalidades (C2, exfiltração, evasão) e nível de sofisticação. Ex: EMOTET (loader), COBALT STRIKE (backdoor/C2), RYUK (ransomware)." },
    ],
    challenges:[
      { type:"complete", sentence:"O Mandiant usa a designação ____ para atores financeiramente motivados, como o FIN7.", blank:"FIN", hint:"FIN (Financially Motivated) é a designação para atores com objetivo financeiro. APT = state-sponsored. UNC = não categorizado ainda. TTPs são técnicas, não designação de atores. Ex: FIN7 é um grupo criminoso que ataca varejo e hotelaria.", options:["FIN","APT","UNC","TTP"] },
      { type:"truefalse", statement:"Quando a origem de um grupo UNC é confirmada, o Mandiant reclassifica para APT ou FIN.", answer:true, hint:"UNC = Uncategorized. Quando análise confirma origem ou motivação, o Mandiant reclassifica o grupo: UNC2452 → APT29 quando confirmado ser russo. A transição requer evidência técnica e geopolítica suficiente." },
      { type:"truefalse", statement:"APT29 é um exemplo de ator financeiramente motivado.", answer:false, hint:"Falso. APT29 (Cozy Bear) é um ator state-sponsored atribuído ao SVR russo, com motivação de espionagem. Atores financeiramente motivados recebem designação FIN (ex: FIN8, FIN7). APT = Advanced Persistent Threat, tipicamente estatal." },
    ]
  },
  { id:3, icon:"🔗", title:"GTI no Google SecOps — Integração",
    cards:[
      { q:"Como o GTI enriquece eventos no Google SecOps?", a:"Via GTI Feed integrado: eventos UDM com IPs, domínios ou hashes são comparados automaticamente com a base GTI. Campos adicionados via security_result: $e.security_result.threat_feed_name, $e.security_result.confidence_score, $e.security_result.severity, $e.security_result.category_details. Sem necessidade de lookup manual." },
      { q:"Como usar IOCs do GTI em regras YARA-L?", a:"Acesse via campos de threat enrichment no UDM:\n\nrule gti_malicious_ip {\n  meta: author = \"SecOps Quest\"\n  events:\n    $e.metadata.event_type = \"NETWORK_CONNECTION\"\n    $e.security_result.confidence_score > 80\n    $e.security_result.severity = \"HIGH\"\n  condition:\n    $e\n}" },
      { q:"O que é o GTI Applied Threat Intelligence?", a:"Camada de inteligência contextual do GTI diretamente nas detecções do SecOps: regras curadas pelo Mandiant (Mandiant-authored detections), IOC matches automáticos, priorização de alertas baseada em relevância do ator para o setor. Transforma threat intel de relatório em ação operacional automatizada." },
      { q:"O que é Threat Landscape no GTI?", a:"Visão estratégica das ameaças ativas relevantes para o contexto do cliente: atores mais ativos no setor/região, malware em crescimento, campanhas recentes. No onboarding GTI (Step 1.3), configura-se o Threat Landscape para filtrar inteligência relevante para a organização — setor, país, tecnologias usadas." },
    ],
    challenges:[
      { type:"complete", sentence:"O campo UDM que armazena o score de confiança do GTI é $e.security_result.____.", blank:"confidence_score", hint:"confidence_score indica o grau de certeza que aquele IOC é malicioso (0-100). Fica em $e.security_result.confidence_score. Outros campos: $e.security_result.severity (LOW/MEDIUM/HIGH/CRITICAL), $e.security_result.threat_feed_name, $e.security_result.category_details. Ref: cloud.google.com/chronicle/docs/reference/udm-field-list", options:["confidence_score","trust_level","ioc_score","threat_level"] },
      { type:"complete", sentence:"O GTI Onboarding Step ____ configura o Threat Landscape da organização por setor e região.", blank:"1.3", hint:"Step 1.3 configura o Threat Landscape — filtragem de inteligência relevante por setor, região e tecnologia. Step 1.1 é o onboarding inicial, 1.2 é admin setup de usuários e permissões.", options:["1.3","1.1","1.2","2.0"] },
      { type:"truefalse", statement:"Para usar IOCs do GTI em YARA-L, é necessário fazer lookup manual em cada regra.", answer:false, hint:"Falso. O GTI Feed injeta automaticamente os metadados de threat intelligence nos eventos UDM. A regra acessa diretamente via campos $e.metadata.threat.* sem precisar de referência a external feeds ou UDTAs." },
    ]
  },
  { id:4, icon:"📊", title:"Análise de Artefatos com VirusTotal",
    cards:[
      { q:"Quais tipos de artefatos o VirusTotal analisa?", a:"4 tipos principais: Arquivos (executáveis, PDFs, scripts — análise estática + dinâmica), URLs (phishing, C2, malware delivery), Domínios (histórico, DNS, categorização), Endereços IP (geolocalização, reputação, histórico de resoluções). Cada artefato recebe detection ratio (ex: 45/72 engines) e contexto de comportamento." },
      { q:"O que é o VT Score e como interpretar?", a:"Detection ratio: número de engines que detectaram / total de engines. Ex: 45/72. Interpretação: 0/72 = limpo ou novo malware (FUD), 1-5/72 = suspeito (pode ser FP), >10/72 = provável malware, >30/72 = confirmado malicioso. Sempre combinar com contexto: sandbox, first submission date, community comments." },
      { q:"O que é o VTAPI e como integrar com SecOps?", a:"API do VirusTotal para automação: busca de hashes, IPs, domínios e URLs programaticamente. No Google SecOps SOAR, existe integração nativa via Google Threat Intelligence connector — playbooks podem enriquecer automaticamente IOCs de alertas consultando o VT. Retorna: score, tags, sandbox report, related files." },
      { q:"O que é o VT Livehunt?", a:"Sistema de YARA no VirusTotal: cria regras YARA que monitoram novos arquivos submetidos ao VT em tempo real. Quando um arquivo novo corresponde à regra, notificação automática. Usado para: detecção de variantes de malware conhecido, monitoramento de implants específicos, hunting de TTPs de atores. Gratuito até limite de regras ativas." },
    ],
    challenges:[
      { type:"complete", sentence:"Um arquivo com 2/72 detecções no VirusTotal deve ser tratado como ____, nunca ignorado.", blank:"suspeito", hint:"2/72 é suspeito. Pode ser malware novo (FUD — Fully Undetectable) ou falso positivo. Sempre combinar com: data de primeira submissão, comentários da comunidade, análise de sandbox, contexto de onde o arquivo foi encontrado. Nunca confiar só no número.", options:["suspeito","limpo","malicioso","ignorado"] },
      { type:"truefalse", statement:"VT Livehunt permite criar regras YARA que monitoram novos arquivos submetidos ao VirusTotal em tempo real.", answer:true, hint:"Correto. VT Livehunt executa regras YARA em novos arquivos submetidos ao VT. Ideal para hunting de variantes de malware ou implants específicos. VT Retrohunt aplica as mesmas regras no histórico de arquivos já submetidos." },
      { type:"complete", sentence:"O VT ____ aplica regras YARA no histórico de arquivos já submetidos ao VirusTotal.", blank:"Retrohunt", hint:"Retrohunt = análise retroativa. Livehunt = tempo real, novos arquivos. Ambos usam sintaxe YARA. Retrohunt é útil para descobrir amostras antigas de um malware que você acabou de identificar. Ref: support.virustotal.com/hc/en-us/articles/360001297797", options:["Retrohunt","Backhunt","Replayhunt","Archivehunt"] },
    ]
  },
  { id:5, icon:"🏹", title:"Threat Hunting com GTI",
    cards:[
      { q:"O que é Threat Hunting e como o GTI potencializa?", a:"Busca proativa de ameaças não detectadas pelos controles automatizados. Com GTI: hipóteses baseadas em inteligência real de atores (TTPs conhecidas do APT relevante para o setor), IOCs de campanhas recentes usados como ponto de partida, Mandiant-authored YARA rules para hunting de implants específicos." },
      { q:"O que são TTPs e como usar no hunting?", a:"TTPs = Tactics, Techniques and Procedures (MITRE ATT&CK). Tática: objetivo (ex: Persistence), Técnica: método (ex: T1053 Scheduled Task), Procedure: implementação específica do ator. Hunting por TTP é mais resiliente que por IOC — IOCs mudam a cada campanha, TTPs são mais estáveis e reutilizadas." },
      { q:"O que é Diamond Model of Intrusion?", a:"Framework de análise de intrusões com 4 vértices: Adversário (quem), Capacidade (como — malware, exploit), Infraestrutura (onde — IPs, domínios), Vítima (alvo). GTI mapeia todos os 4 vértices para cada campanha. Mudar um vértice é custoso para o atacante — pivot de infraestrutura → novos IOCs relacionados." },
      { q:"Como priorizar IOCs no SecOps usando GTI?", a:"Hierarquia de confiança: 1) IOCs de campanha ativa contra seu setor (máxima prioridade), 2) IOCs de ator conhecido relevante, 3) IOCs de campanha genérica, 4) IOCs de feeds públicos sem contexto. No YARA-L: combine $e.metadata.threat.confidence_score > 80 com $e.metadata.threat.category = \"MALWARE\" para alertas de alta fidelidade." },
    ],
    challenges:[
      { type:"complete", sentence:"Hunting por TTP é mais duradouro que por IOC porque TTPs são mais ____ entre campanhas.", blank:"estáveis", hint:"IOCs (IPs, domínios, hashes) mudam frequentemente — atacantes rotacionam infraestrutura. TTPs são muito mais estáveis e reutilizadas entre campanhas. Ex: um APT sempre usa living-off-the-land — detectar pelo comportamento é mais duradouro que pelo hash do implant.", options:["estáveis","públicos","raros","complexos"] },
      { type:"complete", sentence:"No Diamond Model, o vértice ____ representa o malware e exploits usados pelo atacante.", blank:"Capacidade", hint:"Capacidade = como o ataque é executado: malware, exploits, ferramentas. Adversário = quem ataca. Infraestrutura = IPs, domínios, servidores usados. Vítima = alvo. Mudar infraestrutura é barato; desenvolver nova Capacidade (novo malware) é muito mais custoso.", options:["Capacidade","Adversário","Infraestrutura","Vítima"] },
      { type:"truefalse", statement:"IOCs de campanhas ativas contra o mesmo setor da organização devem ter maior prioridade de investigação.", answer:true, hint:"Correto. IOC com contexto de campanha ativa no mesmo setor indica probabilidade real de targeting. IOCs genéricos de feeds públicos sem contexto têm alta taxa de falso positivo. GTI permite filtrar por setor e região para priorização inteligente." },
    ]
  },
  { id:6, icon:"⚙️", title:"GTI Admin Setup e Onboarding",
    cards:[
      { q:"Quais são os primeiros passos do GTI Onboarding (Step 1.1)?", a:"1) Ativar licença GTI no Google Cloud Console, 2) Configurar SSO e autenticação, 3) Adicionar usuários e definir roles (Analyst, Threat Hunter, Admin), 4) Conectar ao tenant Google SecOps via integração nativa, 5) Verificar GTI Feed ativo nos eventos UDM. O onboarding conecta GTI à sua instância SecOps automaticamente." },
      { q:"Quais roles existem no GTI e suas permissões?", a:"Admin: configuração completa, gestão de usuários, acesso a todas as features. Threat Hunter: acesso a Livehunt, Retrohunt, análise profunda de malware, reports completos. Analyst: acesso a lookup de IOCs, reports básicos, integração SOAR. Viewer: leitura somente. Roles definem acesso às features premium e volume de consultas via API." },
      { q:"O que configurar no GTI Admin Setup (Step 1.2)?", a:"1) Definir Collections — grupos de IOCs específicos para o contexto, 2) Configurar notificações de novos IOCs relevantes, 3) Ajustar Threat Landscape (setor, região, tecnologias), 4) Configurar integrações SOAR (VT connector, GTI connector), 5) Definir thresholds de confidence score para alertas automáticos no SecOps." },
      { q:"Como funciona a integração GTI + SOAR no SecOps?", a:"Via Google Threat Intelligence connector no SecOps SOAR: playbooks podem chamar automaticamente VT/GTI para enriquecer IOCs de alertas. Fluxo: alerta criado → playbook extrai IOCs → consulta GTI API → adiciona score/contexto ao case → triage automático baseado no resultado. Reduz tempo de análise manual de IOCs." },
    ],
    challenges:[
      { type:"complete", sentence:"O role GTI ____ tem acesso ao Livehunt e Retrohunt para hunting avançado.", blank:"Threat Hunter", hint:"Threat Hunter tem acesso às features avançadas: Livehunt (YARA em tempo real), Retrohunt (YARA em histórico), análise profunda de malware e relatórios completos. Analyst tem acesso básico. Admin tem acesso a tudo + configuração.", options:["Threat Hunter","Analyst","Viewer","Admin"] },
      { type:"truefalse", statement:"O GTI Feed precisa ser configurado manualmente para cada regra YARA-L no SecOps.", answer:false, hint:"Falso. O GTI Feed é ativado uma vez na integração GTI + SecOps e passa a enriquecer automaticamente todos os eventos UDM. Regras YARA-L acessam os campos $e.metadata.threat.* sem configuração adicional por regra." },
      { type:"complete", sentence:"No GTI Onboarding Step 1.3, configura-se o ____ para filtrar inteligência relevante por setor e região.", blank:"Threat Landscape", hint:"Threat Landscape define o contexto da organização: setor (finance, healthcare, government...), região geográfica, tecnologias usadas. Com isso o GTI prioriza e filtra feeds e relatórios de inteligência relevantes para aquela organização específica.", options:["Threat Landscape","Threat Feeds","IOC Filters","Intel Dashboard"] },
    ]
  },
  { id:7, icon:"🎯", title:"Casos de Uso Avançados",
    cards:[
      { q:"Como usar GTI para enriquecer alertas de phishing?", a:"Fluxo: 1) Alerta de e-mail suspeito no SecOps, 2) Playbook SOAR extrai URL/domínio/sender IP, 3) Consulta GTI/VT → categoria PHISHING? confidence > 80?, 4) Se positivo → auto-close como confirmed ou escalar com contexto completo (campanha associada, outros alvos). Reduz triage de phishing de 15 min para <1 min." },
      { q:"Como correlacionar eventos com ATPs usando GTI?", a:"Na regra YARA-L, combine enriquecimento GTI com padrões comportamentais: events com IOC match GTI + evento de execução suspeita = alta confiança de comprometimento. Ex: $e.metadata.threat.confidence_score > 90 AND $e.target.process.command_line matches /powershell.*-enc/. Reduz FP por contexto duplo." },
      { q:"O que é a GTI Collection e como usar?", a:"Grupo curado de IOCs relacionados a um tema: ator, campanha, malware family, setor. Ex: Collection 'APT41 Infrastructure', 'Ransomware C2 IPs'. No SecOps SOAR, playbooks podem verificar se IOC pertence a uma Collection específica. Útil para: monitoramento de ator específico relevante para o setor da organização." },
      { q:"Como medir a efetividade do GTI no SOC?", a:"KPIs principais: 1) IOC match rate (% de alertas com hit GTI vs total), 2) True positive rate por confidence score (IOCs > 80 = quantos % reais), 3) MTTD (Mean Time to Detect) antes/depois do GTI Feed, 4) Alertas auto-fechados por GTI vs escalados, 5) Hunting leads gerados por GTI. Reportar mensalmente." },
    ],
    challenges:[
      { type:"complete", sentence:"Combinar GTI enrichment com YARA-L comportamental reduz ____ com contexto duplo.", blank:"falsos positivos", hint:"IOC match sozinho pode gerar FP. Comportamento suspeito sozinho também. A combinação — IOC com alta confiança GTI + padrão comportamental anômalo — reduz drasticamente FP. Context-based detection é o padrão recomendado pelo Mandiant.", options:["falsos positivos","regras","alertas","logs"] },
      { type:"truefalse", statement:"Uma GTI Collection pode ser usada em playbooks SOAR para verificar se um IOC pertence a um grupo de ameaças específico.", answer:true, hint:"Correto. Collections permitem verificação contextual: 'Este IP pertence à infraestrutura conhecida do APT41?' via API GTI. Playbooks SOAR podem usar isso para roteamento inteligente: IOC de Collection de ator estatal → escalar para time de threat intel imediatamente." },
      { type:"complete", sentence:"MTTD significa Mean Time to ____, métrica que mede o tempo entre o comprometimento e a detecção.", blank:"Detect", hint:"MTTD (Mean Time to Detect) é uma das principais métricas de SOC. Com GTI Feed e enriquecimento automático, o MTTD tende a diminuir pois IOCs são correlacionados em tempo real sem análise manual. Outra métrica importante: MTTR (Mean Time to Respond).", options:["Detect","Defend","Deploy","Determine"] },
    ]
  },
];

export const M7_FINAL_CHALLENGE = {
  title:"GTI Master — Desafio Final",
  description:"Você recebe um alerta no SecOps: um host interno está se comunicando com um IP externo. O GTI Feed marcou o IP com confidence_score=95, category='MALWARE', e o Mandiant associa esse IP à infraestrutura do APT41 em campanha ativa contra o setor de tecnologia. Demonstre seu conhecimento em GTI para tomar as decisões corretas.",
  questions:[
    { type:"mcq", question:"Qual a primeira ação ao ver confidence_score=95 e APT41 no alerta?", options:["Fechar o alerta — pode ser falso positivo","Iniciar playbook de contenção e notificar o time de Threat Intel","Aguardar mais eventos antes de agir","Bloquear o IP no firewall sem mais análise"], answer:1, hint:"Confidence 95 + APT41 ativo no setor = alta prioridade. Ação imediata: iniciar contenção (isolar host) + notificar Threat Intel (APT state-sponsored exige resposta coordenada). Fechar seria grave descuido. Bloquear IP sem isolar o host não impede C2 por outros canais." },
    { type:"mcq", question:"Para confirmar que o APT41 está ativo na organização, qual fonte GTI consultar?", options:["VirusTotal detection ratio do IP","Mandiant profile do APT41 + Collection da campanha ativa","GTI Threat Landscape configurado","Log de acesso do firewall"], answer:1, hint:"Mandiant APT41 profile contém TTPs, malware usado, infraestrutura histórica e campanhas ativas. A Collection da campanha específica lista todos os IOCs relacionados. Combinar os dois confirma se o padrão observado bate com o MO do APT41 documentado pelo Mandiant." },
    { type:"truefalse", statement:"Após confirmar APT41, você deve atualizar as regras YARA-L com IOCs da campanha para detectar movimentação lateral.", answer:true, hint:"Correto. Operacionalizar a inteligência: adicionar IOCs da campanha APT41 às regras existentes para detectar movimentação lateral, beaconing de outros hosts, staging de dados. GTI Collections facilitam isso — exportar IOCs da campanha diretamente para o SecOps." },
    { type:"mcq", question:"Qual campo YARA-L usar para filtrar eventos com IOC de campanha APT41 de alta confiança?", options:["$e.metadata.threat.actor_name = 'APT41'","$e.metadata.threat.confidence_score > 90","$e.principal.ip in %gti_apt41","Ambos A e B combinados"], answer:3, hint:"A combinação é mais poderosa: confidence_score alto filtra por qualidade do IOC, e actor_name (se disponível no enriquecimento) filtra por atribuição. Usar os dois reduz FP e aumenta especificidade. GTI enrichment popula ambos os campos automaticamente quando o IOC está na base Mandiant." },
  ]
};

// ─── MÓDULO 8 — SOAR & AUTOMAÇÃO ──────────────────────────────────────────────
export const M8_LESSONS = [
  { id:1, icon:"🛡", title:"O que é o SOAR no Google SecOps",
    cards:[
      { q:"O que é SOAR e qual seu papel no Google SecOps?", a:"SOAR = Security Orchestration, Automation and Response. No Google SecOps, o SOAR é a camada de automação que conecta o SIEM (detecção) à resposta. Quando uma regra YARA-L gera um alerta, o SOAR pode automaticamente: enriquecer o IOC via GTI, criar um case, notificar o analista via Slack/e-mail, isolar um host ou abrir um ticket no Jira — tudo sem intervenção manual." },
      { q:"Qual a diferença entre SIEM e SOAR no Google SecOps?", a:"SIEM: coleta, normaliza e detecta — gera alertas baseados em regras YARA-L sobre dados UDM. SOAR: orquestra e automatiza a resposta — executa ações a partir dos alertas do SIEM. No Google SecOps, os dois são integrados nativamente: um alerta SIEM pode disparar automaticamente um playbook SOAR no mesmo painel, sem API externa." },
      { q:"O que é um Case no Google SecOps SOAR?", a:"Unidade de trabalho do analista no SOAR. Agrupa alertas relacionados em uma investigação. Contém: timeline de eventos, IOCs coletados, ações executadas pelos playbooks, notas do analista, SLA de resposta. Cases podem ser criados automaticamente por playbooks ou manualmente. Equivalente a um 'ticket de incidente' com contexto completo de segurança." },
      { q:"O que são Actions e Playbooks no SOAR?", a:"Action: operação atômica executável — consultar VT, bloquear IP no firewall, criar ticket no Jira, enviar e-mail, consultar AD. Cada action pertence a uma Integration. Playbook: fluxo de Actions orquestradas — define quando e como executar cada action, com condições, loops e ramificações. Playbook = automação completa de um processo de resposta." },
    ],
    challenges:[
      { type:"complete", sentence:"No Google SecOps, o ____ detecta ameaças via regras YARA-L e o SOAR orquestra a resposta automatizada.", blank:"SIEM", hint:"SIEM (Security Information and Event Management) coleta, normaliza e detecta via regras sobre dados UDM. SOAR (Security Orchestration, Automation and Response) executa ações a partir desses alertas. No Google SecOps ambos estão integrados nativamente no mesmo painel.", options:["SIEM","EDR","SOAR","GTI"] },
      { type:"complete", sentence:"Um ____ no SOAR agrupa alertas relacionados em uma investigação com timeline, IOCs e ações executadas.", blank:"Case", hint:"Case é a unidade de trabalho do analista no SOAR. Agrupa alertas relacionados, timeline de eventos, IOCs coletados, ações dos playbooks e notas. Pode ser criado automaticamente por playbooks ou manualmente. É o 'ticket de incidente' com contexto completo.", options:["Case","Alert","Ticket","Incident"] },
      { type:"truefalse", statement:"No Google SecOps, o SIEM e o SOAR são módulos separados que exigem integração via API externa.", answer:false, hint:"Falso. No Google SecOps, SIEM e SOAR são integrados nativamente no mesmo painel — um alerta SIEM pode disparar automaticamente um playbook SOAR sem configuração de API externa. Essa integração nativa é um diferencial do Google SecOps em relação a soluções que combinam produtos de fornecedores diferentes." },
    ]
  },
  { id:2, icon:"⚙️", title:"Integrações — Step 1",
    cards:[
      { q:"O que são Integrations no SOAR e como funcionam?", a:"Conectores que permitem ao SOAR se comunicar com sistemas externos. Cada integration contém: autenticação (API key, OAuth, certificado), conjunto de Actions disponíveis e configuração de endpoint. Exemplos: VirusTotal (consulta de IOCs), Jira (criação de tickets), CrowdStrike (isolamento de host), Slack (notificações), Active Directory (query de usuários)." },
      { q:"O que é o Content Hub e como usar?", a:"Repositório oficial de Integrations e Playbooks pré-construídos do Google SecOps. Contém 400+ integrações prontas: EDR, firewall, ticketing, comunicação, cloud. Para usar: acessar SOAR > Content Hub > buscar a integration > instalar > configurar credenciais. Elimina a necessidade de desenvolver integrações do zero para ferramentas comuns." },
      { q:"Como configurar uma Integration no SOAR (Step 1)?", a:"Fluxo de configuração: 1) Content Hub → instalar a integration, 2) Settings > Integrations > selecionar a integration instalada, 3) Preencher parâmetros: API URL, API Key/Token, timeout, 4) Test Connection — validar que o SOAR consegue se comunicar, 5) Salvar. Após configurada, as Actions da integration ficam disponíveis para uso nos Playbooks." },
      { q:"O que são Integration Instances e por que usar múltiplas?", a:"Uma Integration pode ter múltiplas Instances — útil para: ambientes diferentes (prod/homolog), regiões geográficas distintas, tenants de clientes diferentes em MSSP. Ex: VirusTotal com duas instances — uma com API key de quota alta para produção, outra com quota básica para testes. Playbooks podem escolher qual instance usar." },
    ],
    challenges:[
      { type:"complete", sentence:"O repositório oficial de Integrations e Playbooks pré-construídos do Google SecOps chama-se ____.", blank:"Content Hub", hint:"Content Hub contém 400+ integrações prontas para EDR, firewall, ticketing, comunicação e cloud. Elimina o desenvolvimento do zero para ferramentas comuns. Acesso via SOAR > Content Hub > buscar, instalar e configurar credenciais.", options:["Content Hub","Store","Hub","Repository"] },
      { type:"complete", sentence:"Após instalar uma Integration, o primeiro passo de configuração é preencher as credenciais e fazer ____ Connection.", blank:"Test", hint:"Test Connection valida que o SOAR consegue se comunicar com o sistema externo usando as credenciais fornecidas. É obrigatório antes de usar a integration em playbooks. Falha no Test Connection indica problema de credencial, endpoint ou conectividade de rede.", options:["Test","Save","Deploy","Validate"] },
      { type:"truefalse", statement:"Uma mesma Integration no SOAR pode ter múltiplas Instances com configurações diferentes.", answer:true, hint:"Correto. Multiple Instances permitem usar a mesma integration para ambientes diferentes (prod/dev), regiões geográficas distintas ou tenants de clientes em cenário MSSP. Ex: Jira com instance de prod e instance de sandbox para testes de playbooks sem impactar tickets reais." },
    ]
  },
  { id:3, icon:"🔄", title:"Construindo Playbooks — Step 2",
    cards:[
      { q:"Quais são os tipos de blocos em um Playbook SOAR?", a:"5 tipos principais: Action (executa uma operação numa integration), Condition (ramificação if/else baseada em resultado), Loop (itera sobre lista de IOCs ou entidades), Playbook (chama sub-playbook reutilizável), Manual Task (aguarda input humano — aprovação, decisão). Actions podem ser: automated (executa imediatamente) ou manual (aguarda confirmação do analista)." },
      { q:"O que é um Trigger no Playbook SOAR?", a:"Evento que inicia o playbook automaticamente. Tipos: Alert (alert recebido do SIEM com determinados critérios), Case Created, Scheduled (execução periódica — ex: relatório diário), Manual (analista clica em 'Run'). Um mesmo playbook pode ter múltiplos triggers. Alert triggers podem filtrar por: tipo de alerta, severidade, produto de origem, campos específicos do alerta." },
      { q:"O que são Playbook Inputs e Outputs?", a:"Inputs: dados passados para o playbook quando ele inicia — campo do alerta, IOC, entidade. Ex: input 'suspicious_ip' recebe o IP do alerta. Outputs: dados retornados pelo playbook para o case ou para sub-playbooks. Ex: output 'vt_score' retorna o score do VirusTotal. Inputs/Outputs permitem que playbooks sejam reutilizáveis e conectáveis entre si." },
      { q:"O que é a diferença entre Automated e Semi-automated Playbook?", a:"Automated: executa todas as actions sem intervenção — ideal para respostas de baixo risco já validadas (enriquecimento, notificação, abertura de ticket). Semi-automated: pausa em Manual Tasks aguardando aprovação humana antes de ações críticas (isolamento de host, bloqueio de conta, quarentena de arquivo). Best practice: automatizar enriquecimento, manual para contenção em produção." },
      { q:"Como o Gemini ajuda na criação de Playbooks no Google SecOps?", a:"O Gemini está integrado ao SOAR do Google SecOps para acelerar a criação de Playbooks: 1) Geração por linguagem natural — descreva o fluxo em português e o Gemini gera o Playbook com os blocos corretos, 2) Sugestão de próximas Actions com base no contexto do incidente, 3) Explicação de Playbooks existentes em linguagem simples, 4) Detecção de lacunas — o Gemini identifica etapas ausentes no fluxo. Reduz o tempo de criação de Playbooks de horas para minutos." },
    ],
    challenges:[
      { type:"complete", sentence:"O bloco do tipo ____ em um Playbook aguarda input humano como aprovação antes de continuar.", blank:"Manual Task", hint:"Manual Task pausa a execução do playbook aguardando uma ação humana — aprovação, decisão, preenchimento de formulário. Essencial para ações críticas como isolamento de host ou bloqueio de conta, onde uma confirmação humana é necessária antes de impactar produção.", options:["Manual Task","Action","Condition","Loop"] },
      { type:"complete", sentence:"Um Playbook ____ executa todas as actions sem intervenção humana, ideal para enriquecimento e notificações.", blank:"Automated", hint:"Playbooks Automated executam sem pausa para input humano — ideal para: enriquecimento de IOCs via GTI/VT, criação de tickets, notificações via Slack/e-mail, queries no AD. Para ações com impacto em produção (isolar host, bloquear conta), usar Semi-automated com Manual Task.", options:["Automated","Manual","Scheduled","Conditional"] },
      { type:"truefalse", statement:"Um Playbook SOAR pode chamar outro Playbook como sub-playbook, permitindo reutilização de lógica.", answer:true, hint:"Correto. O bloco tipo 'Playbook' dentro de um playbook chama outro playbook como sub-rotina. Permite modularização: playbook de enriquecimento de IP chamado por vários playbooks diferentes. Best practice: criar playbooks atômicos (enrich_ip, enrich_hash, notify_slack) e compô-los em playbooks maiores." },
      { type:"complete", sentence:"O ____ está integrado ao SOAR do Google SecOps e permite criar Playbooks a partir de descrições em linguagem natural.", blank:"Gemini", hint:"O Gemini integrado ao Google SecOps SOAR permite: gerar playbooks por linguagem natural, sugerir próximas actions com base no contexto do incidente, explicar playbooks existentes em linguagem simples e detectar lacunas no fluxo. Reduz o tempo de criação de playbooks de horas para minutos.", options:["Gemini","ChatGPT","Copilot","BARD"] },
    ]
  },
  { id:4, icon:"🔍", title:"Cases, Alertas e SLA",
    cards:[
      { q:"Como funciona o fluxo SIEM → SOAR no Google SecOps?", a:"Fluxo completo: 1) Evento chega via feed/forwarder → normalizado em UDM, 2) Regra YARA-L detecta padrão → gera Alerta, 3) Alerta pode criar ou associar a um Case automaticamente, 4) Case criado dispara playbook via trigger 'Alert', 5) Playbook enriquece, notifica e executa ações, 6) Analista revisa o Case com contexto completo já coletado pelo playbook." },
      { q:"O que é SLA no contexto de Cases SOAR?", a:"Service Level Agreement — prazo máximo para detecção e resposta a incidentes. No SOAR: cada Case tem um SLA configurado por severidade — os valores são definidos pela organização (ex: Critical=1h, High=4h, Medium=24h são referências comuns, mas totalmente configuráveis). O SOAR monitora e alerta quando um Case está próximo de violar o SLA. Relatórios de SLA compliance são usados para demonstrar eficiência do SOC em auditorias e contratos." },
      { q:"O que são Entities no SOAR e como usar em Playbooks?", a:"Entidades são os objetos extraídos do alerta: IP, domínio, hash, usuário, hostname. O SOAR extrai automaticamente entidades dos campos UDM do alerta. Playbooks operam sobre entidades — ex: para cada IP suspeito no alerta, consultar VirusTotal. Loop sobre entities permite processar múltiplos IOCs do mesmo alerta sem duplicar lógica." },
      { q:"O que são Insights e Comments nos Cases?", a:"Insights: observações automáticas geradas por playbooks — 'IP 1.2.3.4: score VT 85/100, categoria MALWARE' adicionado automaticamente ao Case. Comments: notas manuais do analista. Ambos constroem a timeline de investigação do Case. Insights estruturados permitem que outros analistas entendam o contexto sem reler logs — essencial em handoffs de turno." },
    ],
    challenges:[
      { type:"complete", sentence:"No Google SecOps, quando uma regra YARA-L detecta um padrão, ela gera um ____ que pode disparar um Playbook.", blank:"Alerta", hint:"O fluxo é: evento UDM → regra YARA-L detecta → Alerta gerado → Case criado → Playbook disparado via trigger. O Alerta carrega o contexto do evento (campos UDM, severidade, regra que disparou) que o Playbook usa como input para enriquecimento e resposta.", options:["Alerta","Case","Log","Insight"] },
      { type:"complete", sentence:"O prazo máximo para resposta a incidentes por severidade é definido pelo ____ de cada Case.", blank:"SLA", hint:"SLA (Service Level Agreement) define prazos por severidade configurados pela organização. Valores de referência: Critical=1h, High=4h, Medium=24h — mas são totalmente customizáveis no Google SecOps SOAR. O SOAR monitora e alerta quando Cases estão próximos de violar o SLA. Compliance de SLA é reportado em auditorias e contratos de serviço.", options:["SLA","Priority","Timer","Deadline"] },
      { type:"truefalse", statement:"Entities no SOAR são extraídas automaticamente dos campos UDM do alerta, como IPs, hashes e usuários.", answer:true, hint:"Correto. O SOAR extrai automaticamente entidades dos campos UDM: IP → IP entity, domain → domain entity, MD5/SHA256 → file hash entity, username → user entity. Playbooks podem usar Loop sobre entities para processar múltiplos IOCs do mesmo alerta sem duplicar lógica de enriquecimento." },
    ]
  },
  { id:5, icon:"📝", title:"Custom Integrations",
    cards:[
      { q:"Quando criar uma Custom Integration no SOAR?", a:"Quando a ferramenta necessária não existe no Content Hub. Casos comuns: sistemas internos da organização (CMDB proprietário, sistema de identidade legado), APIs específicas do cliente em projetos MSSP, ferramentas de nicho não cobertas pelo Content Hub. Custom integrations usam Python SDK do Google SecOps SOAR e seguem a mesma estrutura das integrações nativas." },
      { q:"Como funciona o SDK de Custom Integration do SOAR?", a:"Baseado em Python. Estrutura: Integration Class (define nome, descrição, configurações de conexão), Action Methods (funções Python que executam operações — cada método vira uma Action disponível no playbook), Connector (opcional — busca alertas de fontes externas periodicamente). O SDK valida inputs/outputs, gerencia autenticação e serializa resultados para o SOAR." },
      { q:"O que é um Connector no SOAR?", a:"Componente que busca alertas de fontes externas para o SOAR — diferente de Integration que responde a chamadas dos playbooks. Connectors ficam rodando periodicamente (a cada X minutos) e ingestam alertas de: SIEMs externos, EDRs sem integração nativa, sistemas de ticketing, APIs proprietárias. Cada alerta ingestado pode disparar playbooks automaticamente." },
      { q:"Como testar e publicar uma Custom Integration?", a:"Fluxo: 1) Desenvolver localmente com Python SDK, 2) IDE Integration — testar actions individualmente no SOAR UI sem publicar, 3) Subir o arquivo ZIP da integration no SOAR, 4) Configurar instância com credenciais de teste, 5) Test Connection, 6) Executar cada action com inputs reais, 7) Publicar para uso em playbooks de produção. Versionamento via ZIP com número de versão." },
    ],
    challenges:[
      { type:"complete", sentence:"Custom Integrations no SOAR são desenvolvidas usando ____ e o SDK oficial do Google SecOps.", blank:"Python", hint:"O SDK de Custom Integration do Google SecOps SOAR é baseado em Python. Define Integration Class, Action Methods (cada método vira uma Action) e opcionalmente Connectors. Segue a mesma estrutura das integrações nativas do Content Hub.", options:["Python","JavaScript","Go","Java"] },
      { type:"complete", sentence:"Um ____ no SOAR busca alertas de fontes externas periodicamente, diferente de uma Integration que responde a playbooks.", blank:"Connector", hint:"Connector fica rodando em loop (ex: a cada 5 min) buscando alertas em SIEMs externos, EDRs, APIs proprietárias. Cada alerta ingestado pode disparar playbooks. Integration responde a chamadas dos playbooks. Ambos usam o Python SDK, mas têm propósitos distintos.", options:["Connector","Integration","Trigger","Action"] },
      { type:"truefalse", statement:"Uma Custom Integration no SOAR segue a mesma estrutura das integrações nativas do Content Hub.", answer:true, hint:"Correto. Custom integrations usam o mesmo Python SDK e seguem a mesma estrutura: Integration Class, Action Methods, configuração de conexão. Após publicadas, aparecem no SOAR exatamente como integrações nativas — os playbooks não distinguem entre nativa e custom." },
    ]
  },
  { id:6, icon:"📊", title:"Métricas e Boas Práticas",
    cards:[
      { q:"Quais métricas de SOAR acompanhar no SOC?", a:"KPIs essenciais: MTTD (Mean Time to Detect — do evento ao alerta), MTTR (Mean Time to Respond — do alerta ao encerramento do Case), Automation Rate (% de Cases fechados sem intervenção manual), SLA Compliance (% de Cases resolvidos dentro do prazo), False Positive Rate (% de alertas descartados como FP). Meta típica: automation rate > 70%, SLA compliance > 95%." },
      { q:"Quais são as melhores práticas para Playbooks SOAR?", a:"1) Modularização: playbooks atômicos reutilizáveis (enrich_ip, notify_slack) compostos em playbooks maiores. 2) Idempotência: executar o playbook duas vezes não duplica ações. 3) Error handling: tratar falhas de API — retry, fallback, notificação. 4) Logging de Insights: registrar cada decisão no Case. 5) Manual gates para ações críticas. 6) Testar em staging antes de produção." },
      { q:"O que é Alert Triage Automation e como implementar?", a:"Automação da triagem inicial de alertas para reduzir carga do analista. Fluxo: alerta chega → playbook enriquece IOCs (GTI, VT) → calcula score de risco combinado → se score < threshold: auto-close como FP com justificativa → se score > threshold: escalar para analista com contexto completo. Reduz volume de alertas manuais em 60-80% em implementações bem calibradas." },
      { q:"Como estruturar Playbooks para MSSP multi-tenant?", a:"Desafios MSSP: múltiplos clientes com ferramentas diferentes, segmentação de dados, customização por cliente. Soluções: Integration Instances por cliente (cada cliente tem sua instance do Jira/Slack), Playbooks com input 'tenant_id' para roteamento, Custom Fields nos Cases para identificar cliente, Scoping de acesso por analista. Google SecOps suporta multi-tenancy nativo." },
    ],
    challenges:[
      { type:"complete", sentence:"A métrica ____ mede o percentual de Cases encerrados sem intervenção manual no SOC.", blank:"Automation Rate", hint:"Automation Rate = Cases auto-fechados / total de Cases. Meta típica > 70%. Complementar: MTTD (tempo até detectar), MTTR (tempo até responder), SLA Compliance (% dentro do prazo). Alta Automation Rate indica playbooks bem calibrados e baixa taxa de FP.", options:["Automation Rate","MTTD","MTTR","SLA Compliance"] },
      { type:"complete", sentence:"Em playbooks SOAR, usar ____ para ações críticas como isolamento de host evita automação de alto risco.", blank:"Manual Task", hint:"Manual Task pausa o playbook aguardando aprovação humana antes de executar ações críticas. Best practice: automatizar enriquecimento e notificações (baixo risco), usar Manual Task para contenção em produção (alto risco). Balanceia velocidade de resposta com controle sobre ações impactantes.", options:["Manual Task","Condition","Loop","Action"] },
      { type:"truefalse", statement:"Alert Triage Automation pode reduzir o volume de alertas manuais para o analista em até 80% quando bem calibrada.", answer:true, hint:"Correto. Implementações bem calibradas de triage automation — enriquecimento + scoring + auto-close de FPs — reduzem 60-80% do volume manual. O analista recebe apenas alertas com contexto completo e score alto, focando energia onde realmente importa. Calibração contínua do threshold é essencial." },
    ]
  },
  { id:7, icon:"🎯", title:"SOAR na Prática — Cenários Reais",
    cards:[
      { q:"Como automatizar resposta a phishing com SOAR?", a:"Playbook de phishing: Trigger (alerta de e-mail suspeito) → Extrair URL/sender IP/attachment hash → Loop: para cada IOC, consultar GTI/VT → Avaliar scores → Se IOC malicioso confirmado: bloquear URL no proxy, quarentena do e-mail, notificar usuário via Slack → Criar Case com timeline completa → Se incerto: Manual Task para analista revisar → Fechar Case com relatório. Tempo: <2 min vs 15 min manual." },
      { q:"Como usar SOAR para resposta a ransomware?", a:"Playbook crítico — semi-automated obrigatório. Trigger (EDR alerta de criptografia em massa) → Enrichment automático: processo pai, hash do malware via VT, blast radius → Manual Task: 'Confirmar isolamento do host [HOSTNAME]?' → Aprovado: isolar host via EDR API, revogar credenciais no AD → Notificar CISO e time de IR → Abrir ticket P1 no Jira → Snapshot forense → Case documenta cada passo para pós-incidente." },
      { q:"Como integrar SOAR com YARA-L para detecção e resposta unificadas?", a:"Fluxo end-to-end: YARA-L detecta (ex: DCsync por conta não-DC) → Alerta com campos UDM preenchidos → Playbook recebe alert.entities (IP, usuário, hostname) → Enriquece cada entity: usuário no AD (privilégios?), IP no GTI (reputação), hostname no EDR (processos ativos) → Monta Case com contexto completo → Manual Task se conta privilegiada → Auto-disable conta se threshold atingido." },
      { q:"O que é o SOAR Workbench e como usar?", a:"Interface de análise de Cases no Google SecOps — painel unificado do analista. Exibe: timeline do Case, entidades extraídas com enriquecimento, ações executadas pelos playbooks, insights gerados, tarefas pendentes (Manual Tasks). O analista pode: executar playbooks manualmente, adicionar IOCs, fechar o Case, escalar. Workbench elimina a necessidade de alternar entre múltiplas ferramentas durante investigação." },
      { q:"Como usar o Gemini para otimizar Playbooks existentes no SOAR?", a:"Casos de uso do Gemini no SOAR além da criação: 1) Análise de Playbook — 'Este playbook cobre todos os passos para resposta a ransomware?' → Gemini identifica lacunas, 2) Tradução de runbook manual para Playbook SOAR automaticamente, 3) Geração de Insights explicativos — Gemini resume o resultado de cada action em linguagem natural no Case, 4) Recomendação de próximas ações durante investigação ativa no Workbench. O Gemini atua como co-piloto do analista durante todo o ciclo de resposta." },
    ],
    challenges:[
      { type:"complete", sentence:"Para resposta a ransomware, o playbook deve ser ____ com Manual Task para confirmar isolamento do host.", blank:"semi-automated", hint:"Ransomware exige contenção rápida mas com confirmação humana — isolar o host errado pode derrubar produção. Semi-automated pausa para aprovação antes de ações críticas. Enriquecimento (hash VT, blast radius) pode ser fully automated. A aprovação humana é o gate para contenção.", options:["semi-automated","automated","manual","scheduled"] },
      { type:"complete", sentence:"O ____ no Google SecOps é o painel unificado onde o analista revisa Cases, entities enriquecidas e tarefas pendentes.", blank:"Workbench", hint:"SOAR Workbench é a interface principal do analista: timeline do Case, entidades com enriquecimento GTI/VT, ações executadas pelos playbooks, Manual Tasks pendentes. Elimina o contexto switching entre SIEM, ticketing, EDR — tudo em um painel. Essencial para reduzir MTTR.", options:["Workbench","Dashboard","Console","Portal"] },
      { type:"truefalse", statement:"Em um playbook de phishing, o enriquecimento de IOCs via GTI/VT pode ser totalmente automatizado sem Manual Task.", answer:true, hint:"Correto. Enriquecimento (consultar GTI, VT, AD) é baixo risco e deve ser automatizado — sem impacto em sistemas de produção. Manual Task é reservada para ações com impacto real: bloquear URL, quarentena de e-mail, revogar credenciais. A automação do enriquecimento libera o analista para decisões de alta complexidade." },
    ]
  },
];

export const M8_FINAL_CHALLENGE = {
  title:"SOAR Master — Desafio Final",
  description:"Você é o engenheiro responsável pelo SOAR do SOC. Um alerta crítico chegou: um host interno executou DCsync, extraindo hashes do Active Directory. O alerta tem confidence_score=98 e o host é um servidor de aplicação — não um Domain Controller. Monte a resposta ideal usando os conceitos de SOAR que você aprendeu.",
  questions:[
    { type:"complete", sentence:"O primeiro bloco do playbook deve ser um ____ automático de entidades: hostname, usuário e IPs do alerta.", blank:"enriquecimento", hint:"Enrichment sempre primeiro — coletar contexto antes de qualquer ação. Para DCsync: hostname no AD (é DC? é servidor de app?), usuário que replicou (conta de serviço? conta humana?), IP de origem no GTI (reputação). O enriquecimento alimenta as Conditions subsequentes.", options:["enriquecimento","bloqueio","notificação","isolamento"] },
    { type:"complete", sentence:"Antes de isolar o servidor de aplicação, o playbook deve incluir uma ____ para aprovação do time de IR.", blank:"Manual Task", hint:"Isolar um servidor de aplicação em produção pode causar indisponibilidade crítica de serviço. Manual Task obrigatória para ações com impacto em produção. O analista recebe contexto completo (enriquecimento já feito) e decide se isola imediatamente ou aplica contenção alternativa.", options:["Manual Task","Condition","Loop","Action"] },
    { type:"truefalse", statement:"Após confirmar DCsync, o playbook deve resetar o krbtgt duas vezes com intervalo para invalidar Golden Tickets existentes.", answer:true, hint:"Correto. Reset do krbtgt (2x com intervalo de 10h mínimo) invalida todos os Golden Tickets emitidos. Esta action pode ser automatizada pois é procedimento padrão documentado após DCsync confirmado — mas deve ter um Insight registrado no Case documentando cada reset com timestamp.", options:[] },
    { type:"complete", sentence:"Todas as ações executadas pelo playbook devem ser registradas como ____ no Case para documentação forense.", blank:"Insights", hint:"Insights são observações estruturadas adicionadas automaticamente ao Case pelo playbook: 'Enriquecimento: host X não é DC, usuário Y tem privilégios de Domain Admin, IP Z score GTI 95'. Constroem a timeline forense do incidente. Essenciais para pós-incidente, auditorias e handoffs de turno.", options:["Insights","Logs","Comments","Actions"] },
  ]
};
